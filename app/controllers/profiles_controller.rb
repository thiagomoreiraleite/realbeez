  ...
  def index
    if params.key?(:search)
      ...
      if params[:search][:query].empty?
        @profiles = policy_scope(Profile.where("statut = ?", "Agent")).order(created_at: :desc).page(params[:page]).per_page(16)
        ...
        if policy_scope(Profile.where("statut = ?", "Agent").near(params[:search][:query],@distance)) != []
          @profiles = policy_scope(Profile.where("statut = ?", "Agent").near(params[:search][:query],@distance)).page(params[:page]).per_page(16)
          @markers = @profiles.where.not(latitude: nil, longitude: nil).map do |profile|
            {
              lat: profile.latitude,
              lng: profile.longitude,
          ...
          @profiles = policy_scope(Profile.where("statut = ?", "Agent").near(params[:search][:query],@distance)).page(params[:page]).per_page(16)
          @profiles_no_result = policy_scope(Profile.where("statut = ?", "Agent").search_agent("paris")).page(params[:page]).per_page(16)
      ...
      if params[:search_all][:query].empty?
        ...
        if policy_scope(Profile.where("statut = ?", "Agent").search_agent(params[:search_all][:query])) != []
          @profiles = policy_scope(Profile.where("statut = ?", "Agent").search_agent(params[:search_all][:query])).page(params[:page]).per_page(16)
          @markers = @profiles.where.not(latitude: nil, longitude: nil).map do |profile|
            {
              lat: profile.latitude,
              lng: profile.longitude,
